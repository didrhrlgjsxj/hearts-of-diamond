게임 메카닉 설명
####앞으로 모든 코드에 수정이 있을 때 마다 이 파일과 각 파일에 대응하는 .txt파일에도 설명을 추가할것 GEMINI에게 부탁... 간단한 수정은 상관없는데 기능을 추가하거나 없애면 그에 대한 최신화를 수행할 것. 꼭꼭 해줄것
================================
...
*   **지휘 부대 (Command Unit)**: **사단(Division)**, **여단(Brigade)**, **대대(Battalion)**는 직접 전투를 수행하지 않는 지휘 부대입니다. 이들의 주된 역할은 하위 부대들을 하나의 단위로 묶어 관리하고, 소속된 부대들에게 지휘 보너스나 특수 능력(향후 추가될 기능)을 제공하는 것입니다.
    ...
    *   **중대 역할**: 휘하의 다른 중대들은 '선발대', '지원대', '유지대' 등의 역할을 부여받습니다.
    *   **동적 진형**: 지휘 부대에 이동 명령을 내리면, 본부 중대가 목표 지점으로 이동하고 나머지 중대들은 본부를 중심으로 자신의 역할에 맞는 진형을 유지하며 자동으로 따라갑니다. 부대는 항상 특정 방향(이동 방향 또는 적 방향)을 바라봅니다.
    *   **유기적 전투**: 전투가 시작되면, 각 전투 중대는 현재 위치에서 가장 가까운 적을 향해 교전하며, 플레이어의 별도 이동 명령이 없는 한 진형을 유지합니다.
